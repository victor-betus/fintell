...
from fintell_package.params import MAXLEN, VECTOR_SIZE


# Goal: train Word2Vec on tokenized sentences, return model
def fit_word2vec(X):
    word2vec = Word2Vec(sentences=X, vector_size=VECTOR_SIZE, min_count=5, window=10)
...
def transform_embedding(X, word2vec):
    ...
    X_pad = pad_sequences(X_embed, dtype='float32', padding='post', maxlen=MAXLEN) # étape 5
    return X_pad
