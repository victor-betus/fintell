...
from fintell_package.params import (
    MODEL_DIR_TOPIC_DL, MODEL_DIR_TOPIC_DL_PLOTS,
    GCS_PROJECT_ID, GCS_BUCKET_NAME, MODEL_TARGET,
    MODEL_TOPIC_DL_NAME, USE_CLASS_WEIGHT, EMBEDDER_NAME
)
...
class GCSCheckpointTopic(Callback):
    def __init__(self, local_path, gcs_checkpoint_prefix="topic_dl_checkpoints"):
        super().__init__()
        self.local_path = local_path
        self.gcs_checkpoint_prefix = gcs_checkpoint_prefix
        ...
        if val_acc > self.best_val_accuracy:
            ...
            if MODEL_TARGET == "gcs":
                upload_file_to_bucket(
                    ...
                    str(self.local_path),
                    f"{self.gcs_checkpoint_prefix}/best_model_{RUN_TIMESTAMP}.keras"
                )
...
def init_model(maxlen, vector_size, model_dl_name=MODEL_TOPIC_DL_NAME, num_classes=7):
    ...
    if model_dl_name == 'lstm':
        ...
        raise ValueError(f"Unknown model_dl_name: '{model_dl_name}'. Choose from: lstm, gru, bilstm, bigru")

    return model


def plot_history(history, plots_dir=MODEL_DIR_TOPIC_DL_PLOTS):
    ...
    plot_path = plots_dir / f'training_history_{RUN_TIMESTAMP}.png'
...
def train_model(X_train, y_train, X_val, y_val, model,
                model_name=MODEL_TOPIC_DL_NAME,
                local_dir=MODEL_DIR_TOPIC_DL,
                gcs_checkpoint_prefix="topic_dl_checkpoints"):
    print(model.summary())

    checkpoint_path = local_dir / f'fintell_{model_name}_{EMBEDDER_NAME}.keras'
    ...
    gcs_ckpt = GCSCheckpointTopic(checkpoint_path, gcs_checkpoint_prefix=gcs_checkpoint_prefix)
